                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="container mx-auto">
                        ...
                        <a href="{{ route('groups.create') }}"
                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mb-4">Create
                            Group</a>
                                ...
                                @forelse($groups as $group)
                                    <tr>
                                        ...
                                        <td class="py-2 px-4 border">
                                            <a href="{{ route('files.index', ['group_id' => $group->id]) }}"
                                            ...
                                            @if ($group->user_id_creater == $userId)
                                                <a href="{{ route('groups.config', $group) }}"
                                                    class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded">config</a>
                                            @endif

                                            @if ($group->user_id_creater == $userId)
                                                <a href="{{ route('report.index', $group) }}"
                                                    class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded">Report</a>
                                            @endif
                                            ...
                                            <form action="{{ route('groups.addUser', $group) }}" method="POST"
                                                ...
                                                <input type="email" name="email" id="email"
                                                    class="mt-1 p-2 border border-gray-300 dark:border-gray-700 rounded-md ">
                                                <button type="submit"
